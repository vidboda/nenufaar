...
do
	if [ "${SEMAPH}" -eq '0' ];then
		...
		if [ "${VC_ONLY}" == 'false' ];then
			...
			echo "COMMAND: ${SRUN_SIMPLE_COMMAND} ${SAMTOOLS} bedcov -Q 30 ${INTERVALS_BED} ${BAM} | ${SORT} -k1,1 -k2,2n -k3,3n | ${AWK} 'BEGIN {OFS=\"\t\"}{a=(\$3-\$2+1);b=($5/a);print \$1,\$2,\$3,\$4,b,\"+\",\"+\"}' > ${OUTPUT_PATH}${RUN_BASEDIR_NAME}/${CURRENT_SAMPLE_BASEDIR_NAME}/${ID}/${CURRENT_SAMPLE_BASEDIR_NAME}_coverage.tsv"
			echo "#############################################################################################"
